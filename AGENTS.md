...
4. **Hanya `fixed` type yang bisa di-request:** `getAvailableComponents` filter `whereNotIn('type', ['hourly', 'presence'])`. Admin UI "Can Request" checkbox juga diganti label "Not available" untuk non-fixed type. Hourly/presence tidak bisa di-request karyawan — kuantitasnya ditentukan otomatis oleh jam kerja / jumlah presensi.
